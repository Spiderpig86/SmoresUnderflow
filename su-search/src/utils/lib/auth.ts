...
export class Auth {
   ...
   * @returns {Promise<IUser>} - promise containing the IUser
   * that can be used to retrieve other data
   * @memberof JWTAuth
   */
  public async validate(token: string): Promise<IUser> {
    try {
      ...
      const user = await this.searchDb.getUser(decode.username);
      ...
      throw new Error('Given token could not be verified for authorization.');
    }
  }
}
